...
test('interact uses only the persisted runtime reference and returns a sanitized reply', async () => {
  ...
  const response = await coordinator.interact({
    run: { id: 'run-1', runtimeReference: { kind: 'pi', id: activated.runtimeSessionId } },
  ...
  assert.deepEqual(await coordinator.interact({ run: { id: 'run-1', runtimeReference: { kind: 'pi', id: 'other' } }, context: {}, message: 'x' }), {
...
test('halt, reconciliation, and close do not leave duplicate or ghost Pi sessions', async () => {
  ...
  await Promise.all([
    coordinator.halt({ id: 'run-1', runtimeReference: { kind: 'pi', id: active.runtimeSessionId } }, { operation: 'run.pause', key: 'pause-1', runtimeReference: { kind: 'pi', id: active.runtimeSessionId } }),
    coordinator.halt({ id: 'run-1', runtimeReference: { kind: 'pi', id: active.runtimeSessionId } }, { operation: 'run.pause', key: 'pause-1', runtimeReference: { kind: 'pi', id: active.runtimeSessionId } })
  ]);
