...
The credential-gated live smoke command is available through `npm run spike:pi-continuity`. Live smoke was not run for this closure because usable credentials were unavailable. This absence is not treated as a failure of the deterministic verification gate, and no provider or user-quality conclusion is drawn from it.
