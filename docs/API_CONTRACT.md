...
- Chat 不仅服务任务推进，也服务困惑处理、生活交流、情绪理解
- 前端不得把聊天区压缩成纯任务日志
- 后端后续如新增消息类型，应优先兼容聊天框式展示
...
- 此刻页输入区需要按“文本 + 语音入口”预留结构
- 后端后续如支持 ASR / 语音输入链路，建议沿用独立能力接口，不与 `/chat` 混写
...
- `/state` 仍是主驾驶舱聚合接口
- 但聊天体验必须优先保证顺滑和陪伴感，不要每次刷新都打断聊天感知
- 任务卡、状态卡、成长卡可局部刷新，不必每次全量重绘
...
```json
{
  ...
  "month_completed": 23,
  "active_days": 5,
  "streak_like_signal": [1, 0, 1, 1, 1, 0, 1],
  "milestone_copy": "这周我们有 5 天都在向前走。"
}
```
