...
让用户可以通过对话管理 Margin 的结构化后台数据，包括记忆、学习线、任务和后续成就记录。
...
- Margin 的建议必须可解释：为什么保留、归档、合并、重命名或删除
...
| `echo_action` | 完成 Margin 建议任务 | 小旗、确认标记 | suggested action done |
...
让 Margin 可以通过对话帮助用户整理后台数据，同时避免误删、误合并和不可追踪的隐式修改。
...
让成就像单机游戏成就一样基于事件解锁，同时保持 Margin 的成长记录气质。
...
在不阻塞终端/API 验证的前提下，重新设计 Margin 的前端页面结构，决定哪些页面保留、合并、新增或删除。
