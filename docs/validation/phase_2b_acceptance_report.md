...
createWebWorkbench opens exactly one configured Margin Core, then composes a trusted Web Gateway, Pi Web Runtime Coordinator, InteractionService, and HTTP adapter. Before the HTTP listener opens, durable running Runs are reconciled to paused. Default local binding is 127.0.0.1. Docker keeps the container listener on 0.0.0.0 but publishes port 3000 on host loopback only (`127.0.0.1:3000:3000`); it does not expose the unauthenticated mutating Workbench on every host interface.

The HTTP host exposes bounded health, command, query, event, and interaction routes. Browser requests traverse the Gateway and cannot submit host-trusted identity, surface, capability, or correlation fields. Known safe Application errors retain their stable codes and retryability; conflicts use 409, storage/runtime failures use 503, and only `version_conflict.details.currentVersion` crosses the browser boundary. The browser never calls Pi directly. Production startup fails with web_assets_missing when web/dist/index.html has not been built; npm start does not build assets.
...
The non-model HTTP E2E uses a temporary directory, real Core, fake runtime, ephemeral HTTP listener, and the real App mounted in JSDOM with the real API client. DOM interactions create and select a Workstream, create/start/pause/resume a Run, switch panels, restart the browser and Web host, reopen authoritative state, display an Artifact, resolve NeedsOwner, and stop the Run. Direct API setup is used only to create Artifact and NeedsOwner because Phase 2B intentionally provides no browser creation control for those objects. The E2E verifies strict unique increasing Event Cursors and Activity cursor agreement. It observes no echo.sqlite and no second/shadow state store.
...
Phase 2B, including this final fix wave, adds no Core schema migration and changes no Core persistence schema. The authoritative path remains data/terminal-pilot/margin-core.sqlite. data/echo.sqlite remains frozen: it is not migrated, merged, or double written.
...
| npm test | exit 0; 467 passed, 0 failed, 0 cancelled, 0 skipped, 0 todo |
...
| Focused Phase 1/2A/2B suites | exit 0; 28 passed, 0 failed |
