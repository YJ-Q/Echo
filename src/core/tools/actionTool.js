    ...
    try {
      ...
      return await store.transaction(async (tx) => {
        ...
        if (input.operation === 'create') {
          ...
          const status = confirmationRequired && !input.confirmationRef ? 'pending_confirmation' : 'pending';
          ...
          await tx.run(`INSERT INTO margin_actions
            ...
            input.detail || '', status, riskLevel, input.dueAt || null, Number(confirmationRequired), input.confirmationRef || null,
          ...
          if (!current) return fail('action_not_found');
          if (current.project_id !== input.projectId) return fail('cross_project_reference');
          if (current.version !== input.expectedVersion) return fail('version_conflict', { details: { expected: input.expectedVersion, actual: current.version } });
          if (current.status === 'pending_confirmation' && input.operation === 'activate' && !input.confirmationRef) return fail('confirmation_required');
          const nextStatus = TRANSITIONS[current.status]?.[input.operation];
          if (!nextStatus) return fail('invalid_request');
